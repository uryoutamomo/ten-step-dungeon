...
- 難易度3段階と数値調整、効果音の切り替え、端末内の途中保存・再開に対応。
...
20件のテスト、6種類の誤実装検出、通常モード100シードとのんびりモード20シードの自動プレイを確認しています。これは到達可能性・ルールの検証であり、人が遊んだときの面白さや難易度の調査ではありません。実ブラウザーでのクリック通し、スマートフォンの実機表示・タッチ操作は未検証です。
